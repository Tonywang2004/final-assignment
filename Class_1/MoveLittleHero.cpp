...
//倒计时设置
void littleHero::updateTimer(float dt)
{
    // 减少倒计时时间
    countdownSeconds--;

    // 创建一个 Label 显示倒计时
    auto timerLabel = Label::createWithTTF("00:00", "fonts/Marker Felt.ttf", 24);
    timerLabel->setPosition(Vec2(Director::getInstance()->getVisibleSize().width / 2,
        Director::getInstance()->getVisibleSize().height - timerLabel->getContentSize().height));
    this->addChild(timerLabel, 1, "timerLabel");

    // 更新 Label 文本显示剩余时间
    int minutes = countdownSeconds / 60;
    int seconds = countdownSeconds % 60;
    auto _timerLabel = (Label*)this->getChildByName("timerLabel");

    if (_timerLabel)
    {
        char buffer[10];
        sprintf(buffer, "%02d:%02d", minutes, seconds);
        _timerLabel->setString(buffer);
    }

    // 检查倒计时是否结束
    if (countdownSeconds <= 0)
    {
        this->unschedule(CC_SCHEDULE_SELECTOR(littleHero::updateTimer));
        // 在这里添加倒计时结束的逻辑，例如显示游戏结束画面
    }
}

//调用倒计时功能
void littleHero::startCountdown(int totalSeconds)
{
    countdownSeconds = totalSeconds;
    // 每秒更新一次计时器
    this->schedule(CC_SCHEDULE_SELECTOR(littleHero::updateTimer), 1.0f);
}